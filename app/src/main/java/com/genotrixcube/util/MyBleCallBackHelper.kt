...
import com.genotrixcube.util.ConnectionManager.pendingOperation
import com.genotrixcube.util.ConnectionManager.signalEndOfOperation
import com.genotrixcube.util.Constant.Companion.CCC_DESCRIPTOR_UUID
import java.util.*
...
class MyBleCallBackHelper(
   ...
   private var mainServiceUUID: UUID
    ...
    override fun onConnectionStateChange(gatt: BluetoothGatt?, status: Int, newState: Int) {
        ...
        Log.wtf(
            "mulfile",
            "device name :" + gatt!!.device.name + " status q" + status + "  newState :" + newState
        )

            if (status == BluetoothGatt.GATT_SUCCESS) {
                if (newState == BluetoothProfile.STATE_CONNECTED) {

                    Log.wtf("mulfile", "BluetoothGatt.GATT_SUCCESS")
                    this.gatt = gatt
                    device=gatt.device
                ...
                //brodcast device is disconnect
                Log.wtf("mulfile:", "Disconnect device")
                BrodcastDeviceDisconnect(context)
            }

        if (status == 133) {
            Log.wtf("Brodcast error :", "error")

            gatt.close()
            BrodcastDeviceErro("error", context)
        } else {
...
//                        Log.wtf("char","char read :"+title + " type of response ")
    ...
    fun BrodcastDeviceDisconnect(context: Context) {
        try {
    ...
    fun BrodcastDeviceErro(string: String, context: Context) {
        try {

            val intent = Intent(DashboardActivity.intentAction)
            intent.putExtra("deviceError", string)
            LocalBroadcastManager.getInstance(context).sendBroadcast(intent)
